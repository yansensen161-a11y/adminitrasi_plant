...
class UnitAplController extends Controller
{
    /**
    ...
    /**
     * Store a newly created APL record.
     */
    public function store(Request $request): RedirectResponse
    {
        ...
        return back()->with('success', 'Data APL berhasil ditambahkan.');
    }

    /**
     * Update the specified APL record.
     */
    public function update(Request $request, UnitApl $unitApl): RedirectResponse
    {
        $validated = $request->validate([
            'part_number' => ['required', 'string', 'max:255'],
            'depart' => ['nullable', 'string', 'max:255'],
        ...
        $unitApl->update([
            ...
            'price_rate' => $priceRate,
            'amount' => $amount,
            'is_global' => isset($validated['is_global']) ? (bool) $validated['is_global'] : $unitApl->is_global,
            'notes' => $validated['notes'] ?? null,
        ]);

        return back()->with('success', 'Data APL berhasil diperbarui.');
    }

    /**
     * Download Excel template for List Part (APL).
     */
    public function downloadTemplate(): StreamedResponse
    {
        $spreadsheet = new Spreadsheet;
        $sheet = $spreadsheet->getActiveSheet();
        $sheet->setTitle('Template Import Part APL');
        ...
        $samples = [
            ['60201220', 'ENGINE', 'Fuel Strainer Element', 1, 'PCS', '✓', '✓', '✓', '✓', '330.00', 'TIDAK', 'Contoh 330.00 = Rp 330.000'],
            ['60282117', 'ENGINE', 'Fuel Fine Filter Core', 1, 'PCS', '✓', '✓', '✓', '✓', '550.00', 'TIDAK', 'Contoh 550.00 = Rp 550.000'],
            ['60249072', 'ENGINE', 'Diesel Fuel Cleaner', 1, 'PCS', '✓', '✓', '✓', '✓', '350.00', 'TIDAK', ''],
            ['B222100000494', 'ENGINE', 'Oil Filter', 1, 'PCS', '✓', '✓', '✓', '✓', '350.00', 'TIDAK', ''],
            ['A222100000119', 'HYDRAULIC', 'Pilot Filter Element', 1, 'PCS', '✓', '✓', '✓', '✓', '250.00', 'TIDAK', ''],
            ['B222100000640', 'AIR INTAKE', 'Auxiliary Cartridge Of Air Cleaner', 1, 'PCS', '', '', '✓', '✓', '1.350.000', 'TIDAK', 'Contoh format ribuan'],
        ];
        ...
        }, 'Template_Import_List_Part_APL.xlsx');
    }

    /**
     * Import parts into List APL from Excel or CSV.
     */
    public function import(Request $request): RedirectResponse
    {
        ...
        foreach ($headerCandidates as $rIdx => $rawRow) {
            ...
            if ($partIdx !== null) {
                ...
                foreach ($normalized as $idx => $h) {
                    ...
                    } elseif (in_array($h, ['depart', 'kompartemen', 'department', 'bagian', 'dept', 'kategori'])) {
                        $colMap['depart'] = $idx;
                    } elseif (in_array($h, ['description', 'deskripsi', 'namapart', 'itemname', 'keteranganpart', 'partdescription', 'namaitem', 'item'])) {
        ...
        try {
            if ($replaceExisting && $unit) {
                UnitApl::where('unit_id', $unit->id)->delete();
            }
            ...
            return back()->with('success', "Berhasil mengimpor {$importedCount} data Part ke List APL.");
        } catch (\Throwable $e) {
            DB::rollBack();

            return back()->with('error', 'Gagal mengimpor data List APL: '.$e->getMessage());
        }
    }
}
