...
class RequestAssetDisposedController extends Controller
{
    public function index(Request $request)
    {
        ...
        $defaultItems = PlantForm::getRequestAssetDisposedItems();
        $suggestedFormNumber = PlantForm::generateAssetDisposedNumber();
        ...
        $recentForms = PlantForm::with(['unit', 'creator'])
            ->where('form_type', 'REQUEST-ASSET-DISPOSED')
            ->latest('date')
            ->latest('id')
            ->take(20)
            ->get();

        return Inertia::render('RequestAssetDisposed/Index', [
            'units' => $units,
    ...
    {
        return redirect()->route('form-request-asset-disposed.index', $request->query());
    }
    ...
    public function store(Request $request)
    {
        ...
        $validated['form_type'] = 'REQUEST-ASSET-DISPOSED';
        $validated['project_id'] = $validated['project_id'] ?? 'PT Mitra Abadi Mahakam';

        if (empty($validated['form_number'])) {
            $validated['form_number'] = PlantForm::generateAssetDisposedNumber();
        }
        ...
        return redirect()->route('form-request-asset-disposed.index', ['id' => $plantForm->id])
            ->with('success', "Request Asset Disposed {$plantForm->form_number} berhasil disimpan.");
    }

    public function show($id)
    {
        return redirect()->route('form-request-asset-disposed.index', ['id' => $id]);
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        return redirect()->route('form-request-asset-disposed.index', ['id' => $plantForm->id])
            ->with('success', "Request Asset Disposed {$plantForm->form_number} berhasil diperbarui.");
    }
    ...
    public function destroy($id)
    {
        ...
        return redirect()->route('form-request-asset-disposed.index')
    ...
    public function print($id)
    {
        ...
        return Inertia::render('RequestAssetDisposed/Print', [
            'form' => $plantForm,
            'isBlank' => false,
            'defaultItems' => PlantForm::getRequestAssetDisposedItems(),
        ]);
    ...
    public function downloadPdf(Request $request)
    {
        ...
        $items = $selectedForm?->items ?? PlantForm::getRequestAssetDisposedItems();
        $results = $selectedForm?->results_data ?? [];

        $pdf = Pdf::loadView('pdf.request-asset-disposed', [
            'form' => $selectedForm,
            'unit' => $unit,
            'items' => $items,
            'results' => $results,
            'formNumber' => $selectedForm?->form_number ?? ($request->form_number ?? 'DISPOSE/PLT/'.date('Y').'/01'),
        ...
        $filename = 'Request_Asset_Disposed_'.str_replace('/', '_', $selectedForm?->form_number ?? 'BLANK').'_'.date('Ymd_His').'.pdf';
