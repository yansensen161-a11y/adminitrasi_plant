...
class PreReleaseTrackUnitController extends Controller
{
    public function index(Request $request)
    {
        ...
        $defaultItems = PlantForm::getPreReleaseTrackUnitItems();
        $suggestedFormNumber = PlantForm::generateFormNumber('PRT');
        ...
        $recentForms = PlantForm::with(['unit', 'creator'])
            ->where('form_type', 'PRE-RELEASE-TRACK-UNIT')
            ->latest('date')
            ->latest('id')
            ->take(20)
            ->get();

        return Inertia::render('PreReleaseTrackUnit/Index', [
            'units' => $units,
    ...
    {
        return redirect()->route('form-pre-release-track-unit.index', $request->query());
    }
    ...
    public function store(Request $request)
    {
        ...
        $validated['form_type'] = 'PRE-RELEASE-TRACK-UNIT';
        $validated['project_id'] = $validated['project_id'] ?? 'PT Mitra Abadi Mahakam';

        if (empty($validated['form_number'])) {
            $validated['form_number'] = PlantForm::generateFormNumber('PRT');
        }
        ...
        return redirect()->route('form-pre-release-track-unit.index', ['id' => $plantForm->id])
            ->with('success', "Pre Release Check List Track Unit {$plantForm->form_number} berhasil disimpan.");
    }

    public function show($id)
    {
        return redirect()->route('form-pre-release-track-unit.index', ['id' => $id]);
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        return redirect()->route('form-pre-release-track-unit.index', ['id' => $plantForm->id])
            ->with('success', "Pre Release Check List Track Unit {$plantForm->form_number} berhasil diperbarui.");
    }
    ...
    public function destroy($id)
    {
        ...
        return redirect()->route('form-pre-release-track-unit.index')
    ...
    public function print($id)
    {
        ...
        return Inertia::render('PreReleaseTrackUnit/Print', [
            'form' => $plantForm,
            'isBlank' => false,
            'defaultItems' => PlantForm::getPreReleaseTrackUnitItems(),
        ]);
    ...
    public function downloadPdf(Request $request)
    {
        ...
        $items = $selectedForm?->items ?? PlantForm::getPreReleaseTrackUnitItems();
        $results = $selectedForm?->results_data ?? [];

        $pdf = Pdf::loadView('pdf.pre-release-track-unit', [
            'form' => $selectedForm,
            'unit' => $unit,
            'items' => $items,
            'results' => $results,
            'formNumber' => $selectedForm?->form_number ?? ($request->form_number ?? 'PLT/FRM/PRT/001'),
        ...
        $filename = 'Pre_Release_Track_Unit_'.($unit?->code_unit ?? 'BLANK').'_'.date('Ymd_His').'.pdf';
