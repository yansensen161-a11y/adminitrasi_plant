...
class CheckSheetMotorgraderController extends Controller
{
    public function index(Request $request)
    {
        ...
        $defaultItems = PlantForm::getCheckSheetMotorgraderItems();
        $suggestedFormNumber = PlantForm::generateFormNumber('CSM');
        ...
        $recentForms = PlantForm::with(['unit', 'creator'])
            ->where('form_type', 'CHECK-SHEET-MOTORGRADER')
            ->latest('date')
            ->latest('id')
            ->take(20)
            ->get();

        return Inertia::render('CheckSheetMotorgrader/Index', [
            'units' => $units,
    ...
    {
        return redirect()->route('form-check-sheet-motorgrader.index', $request->query());
    }
    ...
    public function store(Request $request)
    {
        ...
        $validated['form_type'] = 'CHECK-SHEET-MOTORGRADER';
        $validated['project_id'] = $validated['project_id'] ?? 'PT Mitra Abadi Mahakam';

        if (empty($validated['form_number'])) {
            $validated['form_number'] = PlantForm::generateFormNumber('CSM');
        }
        ...
        return redirect()->route('form-check-sheet-motorgrader.index', ['id' => $plantForm->id])
            ->with('success', "Check Sheet Service Motorgrader {$plantForm->form_number} berhasil disimpan.");
    }

    public function show($id)
    {
        return redirect()->route('form-check-sheet-motorgrader.index', ['id' => $id]);
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        return redirect()->route('form-check-sheet-motorgrader.index', ['id' => $plantForm->id])
            ->with('success', "Check Sheet Service Motorgrader {$plantForm->form_number} berhasil diperbarui.");
    }
    ...
    public function destroy($id)
    {
        ...
        return redirect()->route('form-check-sheet-motorgrader.index')
    ...
    public function print($id)
    {
        ...
        return Inertia::render('CheckSheetMotorgrader/Print', [
            'form' => $plantForm,
            'isBlank' => false,
            'defaultItems' => PlantForm::getCheckSheetMotorgraderItems(),
        ]);
    ...
    public function downloadPdf(Request $request)
    {
        ...
        $items = $selectedForm?->items ?? PlantForm::getCheckSheetMotorgraderItems();
        $results = $selectedForm?->results_data ?? [];

        $pdf = Pdf::loadView('pdf.check-sheet-motorgrader', [
            'form' => $selectedForm,
            'unit' => $unit,
            'items' => $items,
            'results' => $results,
            'formNumber' => $selectedForm?->form_number ?? ($request->form_number ?? 'PLT/FRM/CSM/001'),
        ...
        $filename = 'Check_Sheet_Service_Motorgrader_'.($unit?->code_unit ?? 'BLANK').'_'.date('Ymd_His').'.pdf';
