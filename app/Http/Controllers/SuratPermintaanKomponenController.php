...
class SuratPermintaanKomponenController extends Controller
{
    public function index(Request $request)
    {
        ...
        $defaultItems = PlantForm::getDefaultSuratPermintaanKomponenItems();
        $suggestedFormNumber = PlantForm::generateMemoKomponenNumber();
        ...
        $recentForms = PlantForm::with(['unit', 'creator'])
            ->where('form_type', 'SURAT-PERMINTAAN-KOMPONEN')
            ->latest('date')
            ->latest('id')
            ->take(20)
            ->get();

        return Inertia::render('SuratPermintaanKomponen/Index', [
            'units' => $units,
    ...
    {
        return redirect()->route('form-surat-permintaan-komponen.index', $request->query());
    }
    ...
    public function store(Request $request)
    {
        ...
        $validated['form_type'] = 'SURAT-PERMINTAAN-KOMPONEN';
        $validated['project_id'] = $validated['project_id'] ?? 'PT Mitra Abadi Mahakam';

        if (empty($validated['form_number'])) {
            $validated['form_number'] = PlantForm::generateMemoKomponenNumber();
        }
        ...
        return redirect()->route('form-surat-permintaan-komponen.index', ['id' => $plantForm->id])
            ->with('success', "Surat Permintaan Komponen {$plantForm->form_number} berhasil disimpan.");
    }

    public function show($id)
    {
        return redirect()->route('form-surat-permintaan-komponen.index', ['id' => $id]);
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        return redirect()->route('form-surat-permintaan-komponen.index', ['id' => $plantForm->id])
            ->with('success', "Surat Permintaan Komponen {$plantForm->form_number} berhasil diperbarui.");
    }
    ...
    public function destroy($id)
    {
        ...
        return redirect()->route('form-surat-permintaan-komponen.index')
    ...
    public function print($id)
    {
        ...
        return Inertia::render('SuratPermintaanKomponen/Print', [
            'form' => $plantForm,
            'isBlank' => false,
            'defaultItems' => PlantForm::getDefaultSuratPermintaanKomponenItems(),
        ]);
