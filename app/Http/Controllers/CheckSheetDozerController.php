...
class CheckSheetDozerController extends Controller
{
    public function index(Request $request)
    {
        ...
        $defaultItems = PlantForm::getCheckSheetDozerItems();
        $suggestedFormNumber = PlantForm::generateFormNumber('CSD');
        ...
        $recentForms = PlantForm::with(['unit', 'creator'])
            ->where('form_type', 'CHECK-SHEET-DOZER')
            ->latest('date')
            ->latest('id')
            ->take(20)
            ->get();

        return Inertia::render('CheckSheetDozer/Index', [
            'units' => $units,
    ...
    {
        return redirect()->route('form-check-sheet-dozer.index', $request->query());
    }
    ...
    public function store(Request $request)
    {
        ...
        return redirect()->route('form-check-sheet-dozer.index', ['id' => $plantForm->id])
            ->with('success', "Check Sheet Service Dozer {$plantForm->form_number} berhasil disimpan.");
    }

    public function show($id)
    {
        return redirect()->route('form-check-sheet-dozer.index', ['id' => $id]);
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        return redirect()->route('form-check-sheet-dozer.index', ['id' => $plantForm->id])
            ->with('success', "Check Sheet Service Dozer {$plantForm->form_number} berhasil diperbarui.");
    }
    ...
    public function destroy($id)
    {
        ...
        return redirect()->route('form-check-sheet-dozer.index')
    ...
    public function print($id)
    {
        ...
        return Inertia::render('CheckSheetDozer/Print', [
            'form' => $plantForm,
            'isBlank' => false,
            'defaultItems' => PlantForm::getCheckSheetDozerItems(),
        ]);
    ...
    public function downloadPdf(Request $request)
    {
        ...
        $items = $selectedForm?->items ?? PlantForm::getCheckSheetDozerItems();
        $results = $selectedForm?->results_data ?? [];

        $pdf = Pdf::loadView('pdf.check-sheet-dozer', [
            'form' => $selectedForm,
            'unit' => $unit,
            'items' => $items,
            'results' => $results,
            'formNumber' => $selectedForm?->form_number ?? ($request->form_number ?? 'PLT/FRM/CSD/001'),
        ...
        $filename = 'Check_Sheet_Service_Dozer_'.($unit?->code_unit ?? 'BLANK').'_'.date('Ymd_His').'.pdf';
