...
class GensetServiceController extends Controller
{
    public function index(Request $request)
    {
        ...
        $defaultItems = PlantForm::getGensetChecklistItems();
        $suggestedFormNumber = PlantForm::generateFormNumber('PM-GEN');
        ...
        $recentForms = PlantForm::with(['unit', 'creator'])
            ->where('form_type', 'PM-GENSET')
            ->latest('date')
            ->latest('id')
            ->take(15)
            ->get();

        return Inertia::render('GensetService/Index', [
            'units' => $units,
    ...
    {
        return redirect()->route('form-service-genset.index');
    }
    ...
    public function store(Request $request)
    {
        ...
        $validated['form_type'] = $validated['form_type'] ?? 'PM-GENSET';

        if (empty($validated['form_number'])) {
            $validated['form_number'] = PlantForm::generateFormNumber('PM-GEN');
        }
        ...
        return redirect()->route('form-service-genset.index', ['id' => $plantForm->id])
            ->with('success', "Form PM Genset {$plantForm->form_number} berhasil disimpan.");
    }
    ...
    {
        return redirect()->route('form-service-genset.index', ['id' => $id]);
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        return redirect()->route('form-service-genset.index', ['id' => $plantForm->id])
            ->with('success', "Form PM Genset {$plantForm->form_number} berhasil diperbarui.");
    }
    ...
    public function print($id)
    {
        ...
        return Inertia::render('GensetService/Print', [
            'form' => $plantForm,
            'isBlank' => false,
            'defaultItems' => PlantForm::getGensetChecklistItems(),
        ]);
    ...
    public function downloadPdf(Request $request)
    {
        ...
        $items = $selectedForm ? $selectedForm->items : PlantForm::getGensetChecklistItems();
        $serviceType = $selectedForm?->service_type ?? ($request->service_type ?? 'A');

        $inspectorName = $selectedForm?->results_data['inspector_name'] ?? ($request->inspector_name ?? '');
        $kmVal = $selectedForm?->results_data['km'] ?? ($request->km ?? '');

        $pdf = Pdf::loadView('pdf.pm-service-sheet-genset', [
            'form' => $selectedForm,
            'unit' => $unit,
            'items' => $items,
            'formNumber' => $selectedForm?->form_number ?? ($request->form_number ?? 'PLT/FRM/PM-GEN/001'),
        ...
        $filename = 'PM_Service_Sheet_Genset_'.($unit?->code_unit ?? 'BLANK').'_'.date('Ymd_His').'.pdf';
