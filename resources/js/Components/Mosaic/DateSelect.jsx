            ...
            options.map(option => {
              return (
                <button
                  key={option.id}
                  tabIndex="0"
                  className={`flex items-center w-full hover:bg-gray-50 dark:hover:bg-gray-700/20 py-1 px-3 cursor-pointer ${option.id === selected && 'text-[#0b5c3e]'}`}
                  onClick={() => { setSelected(option.id); setDropdownOpen(false); }}
                >
                  <svg className={`shrink-0 mr-2 fill-current text-[#0b5c3e] ${option.id !== selected && 'invisible'}`} width="12" height="9" viewBox="0 0 12 9">
