...
export default function Index({
    units = [],
    defaultItems = [],
    suggestedFormNumber = 'PLT/FRM/CSD/001',
    selectedForm = null,
    recentForms = [],
    prefill = {},
}) {
    ...
    const handleResetAll = () => {
        if (window.confirm('Reset semua status checklist form Dozer ini?')) {
    ...
    const handleSave = (e) => {
        ...
        if (!unitId) {
            alert('Pilih unit alat berat Dozer terlebih dahulu!');
            return;
        }

        setIsSaving(true);
        const payload = {
            form_type: 'CHECK-SHEET-DOZER',
        ...
        if (selectedForm && selectedForm.id) {
            router.put(`/form-check-sheet-dozer/${selectedForm.id}`, payload, {
                onSuccess: () => {
                    setIsSaving(false);
                    setNotification('Check Sheet Service Dozer berhasil diperbarui.');
                    setTimeout(() => setNotification(null), 4000);
                },
                onError: () => setIsSaving(false),
            });
        } else {
            router.post('/form-check-sheet-dozer', payload, {
                onSuccess: () => {
                    setIsSaving(false);
                    setNotification('Check Sheet Service Dozer berhasil disimpan.');
                    setTimeout(() => setNotification(null), 4000);
                },
                onError: () => setIsSaving(false),
            });
    ...
    return (
        <AuthenticatedLayout>
            <Head title="Check Sheet Service Dozer" />
            ...
            <div className="py-6 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto space-y-6">
                ...
                <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 bg-slate-900/60 p-5 rounded-2xl border border-slate-800 backdrop-blur-xl">
                    <div>
                        <div className="flex items-center gap-3">
                            <span className="p-2 rounded-xl bg-amber-500/10 text-amber-400 border border-amber-500/20 text-xl font-bold">🚜</span>
                            <div>
                                <h1 className="text-2xl font-black tracking-tight text-white flex items-center gap-2">
                                    CHECK SHEET SERVICE DOZER
                                </h1>
                                <p className="text-xs text-slate-400 font-medium mt-0.5">
                                    Digitalisasi PM Service Checklist Khusus Unit Bulldozer Track
                                </p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="flex flex-wrap items-center gap-2">
                        ...
                        <a
                            href={`/form-check-sheet-dozer/blank-print${unitId ? `?unit_id=${unitId}` : ''}`}
                            target="_blank"
                        ...
                        {selectedForm && (
                            <>
                                <a
                                    href={`/form-check-sheet-dozer/${selectedForm.id}/print`}
                                    target="_blank"
                                    rel="noopener noreferrer"
                                ...
                                <a
                                    href={`/form-check-sheet-dozer/download-pdf?id=${selectedForm.id}`}
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4 bg-slate-900/60 p-5 rounded-2xl border border-slate-800">
                    <div>
                        ...
                        <input
                            ...
                            className="w-full bg-slate-950 border border-slate-700 rounded-xl px-3 py-2 text-xs text-white focus:ring-1 focus:ring-amber-500"
                        />
                    </div>

                    <div>
                        <label className="block text-xs font-semibold text-slate-300 mb-1.5">Pilih Unit Dozer *</label>
                        <select
                            value={unitId}
                        ...
                        <table className="w-full text-left text-xs text-slate-300">
                            <thead className="bg-slate-950 text-slate-400 border-b border-slate-800 uppercase font-bold text-[10px] tracking-wider">
                                <tr>
                                    ...
                                    <th className="px-4 py-2.5">Kompartemen / Komponen Dozer</th>
                                    ...
                                    return (
                                        ...
                                        >
                                            ...
                                            {['250/750', '500', '1000', '2000'].map(intv => {
                                                ...
                                                return (
                                                    <td key={intv} className={`px-2 py-2 text-center ${isSelected ? 'bg-amber-950/30' : ''}`}>
                                                        <button
                                                            type="button"
                                                            ...
                                                            className={`w-6 h-6 rounded-md border text-xs font-bold transition inline-flex items-center justify-center ${
                                                                item.status === 'OK' && isSelected
                                                                    ...
                                                                    : isSelected
                                                                    ? 'bg-amber-950/40 border-amber-600/50 text-amber-400 hover:bg-emerald-600 hover:text-white'
                ...
                {/* Specialized Grid Dozer: Magnetic Plug, Cut Filter, Cylinder, Data Download */}
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {/* Magnetic Plug Card */}
                    <div className="bg-slate-900/60 p-4 rounded-2xl border border-slate-800">
                        <div className="flex items-center justify-between mb-3 border-b border-slate-800 pb-2">
                            <h3 className="text-xs font-bold text-white uppercase tracking-wider flex items-center gap-1.5">
                                <span>🧲</span> Plug Magnet & Catat Dozer
                            </h3>
                        ...
                        <div className="grid grid-cols-2 gap-2.5 text-xs">
                            <div>
                                ...
                                <input
                                    ...
                                    placeholder="Rating partikel..."
                                    className="w-full bg-slate-950 border border-slate-700 rounded-lg px-2.5 py-1.5 text-white"
                                />
                            </div>
                            <div>
                                <label className="text-slate-400 text-[11px] block">Hydraulic</label>
                                <input
                                    type="text"
                                    value={resultsData.mag_hyd || ''}
                                    onChange={e => setResultsData({ ...resultsData, mag_hyd: e.target.value })}
                                    placeholder="Rating partikel..."
                                    className="w-full bg-slate-950 border border-slate-700 rounded-lg px-2.5 py-1.5 text-white"
                                />
                            </div>
                            <div>
                                <label className="text-slate-400 text-[11px] block">Final Drive RH</label>
                                <input
                                    type="text"
                                    value={resultsData.mag_fdrh || ''}
                                    onChange={e => setResultsData({ ...resultsData, mag_fdrh: e.target.value })}
                                    placeholder="Rating partikel..."
                                    className="w-full bg-slate-950 border border-slate-700 rounded-lg px-2.5 py-1.5 text-white"
                                />
                            </div>
                            <div>
                                <label className="text-slate-400 text-[11px] block">Final Drive LH</label>
                                <input
                                    type="text"
                                    value={resultsData.mag_fdlh || ''}
                                    onChange={e => setResultsData({ ...resultsData, mag_fdlh: e.target.value })}
                                    placeholder="Rating partikel..."
                                    className="w-full bg-slate-950 border border-slate-700 rounded-lg px-2.5 py-1.5 text-white"
                                />
                            </div>
                            <div className="col-span-2">
                                <label className="text-slate-400 text-[11px] block">Power Train Case Strainer</label>
                                <input
                                    type="text"
                                    value={resultsData.mag_ptc || ''}
                                    onChange={e => setResultsData({ ...resultsData, mag_ptc: e.target.value })}
                                    placeholder="Rating partikel strainer..."
                                    className="w-full bg-slate-950 border border-slate-700 rounded-lg px-2.5 py-1.5 text-white"
                                />
                            </div>
                        </div>
                    ...
                    <div className="bg-slate-900/60 p-4 rounded-2xl border border-slate-800">
                        <div className="flex items-center justify-between mb-3 border-b border-slate-800 pb-2">
                            <h3 className="text-xs font-bold text-white uppercase tracking-wider flex items-center gap-1.5">
                                <span>✂️</span> Potong / Periksa Saringan Dozer
                            </h3>
                        ...
                        <div className="grid grid-cols-3 gap-2.5 text-xs">
                            ...
                            <div>
                                <label className="text-slate-400 text-[11px] block">Power Train Case</label>
                                <input
                                    type="text"
                                    value={resultsData.cut_ptc || ''}
                                    onChange={e => setResultsData({ ...resultsData, cut_ptc: e.target.value })}
                                    placeholder="Status..."
                                    className="w-full bg-slate-950 border border-slate-700 rounded-lg px-2.5 py-1.5 text-white"
                                />
                            </div>
                    ...
                    <div className="bg-slate-900/60 p-4 rounded-2xl border border-slate-800">
                        <div className="flex items-center justify-between mb-3 border-b border-slate-800 pb-2">
                            <h3 className="text-xs font-bold text-white uppercase tracking-wider flex items-center gap-1.5">
                                <span>🔧</span> Periksa & Catat Cylinder Dozer
                            </h3>
                            <span className="text-[10px] text-blue-400 italic">STP Cylinder</span>
                        </div>
                        <div className="grid grid-cols-2 gap-2.5 text-xs">
                            <div>
                                <label className="text-slate-400 text-[11px] block">Blade Lift RH</label>
                                <input
                                    type="text"
                                    value={resultsData.cyl_blade_lift_rh || ''}
                                    onChange={e => setResultsData({ ...resultsData, cyl_blade_lift_rh: e.target.value })}
                                    placeholder="Kondisi..."
                                    className="w-full bg-slate-950 border border-slate-700 rounded-lg px-2.5 py-1.5 text-white"
                                />
                            </div>
                            <div>
                                <label className="text-slate-400 text-[11px] block">Blade Lift LH</label>
                                <input
                                    type="text"
                                    value={resultsData.cyl_blade_lift_lh || ''}
                                    onChange={e => setResultsData({ ...resultsData, cyl_blade_lift_lh: e.target.value })}
                                    placeholder="Kondisi..."
                                    className="w-full bg-slate-950 border border-slate-700 rounded-lg px-2.5 py-1.5 text-white"
                                />
                            </div>
                            <div>
                                <label className="text-slate-400 text-[11px] block">Blade Tilt LEFT</label>
                                <input
                                    type="text"
                                    value={resultsData.cyl_blade_tilt_lh || ''}
                                    onChange={e => setResultsData({ ...resultsData, cyl_blade_tilt_lh: e.target.value })}
                                    placeholder="Kondisi..."
                                    className="w-full bg-slate-950 border border-slate-700 rounded-lg px-2.5 py-1.5 text-white"
                                />
                            </div>
                            <div>
                                <label className="text-slate-400 text-[11px] block">Blade Tilt RIGHT</label>
                                <input
                                    type="text"
                                    value={resultsData.cyl_blade_tilt_rh || ''}
                                    onChange={e => setResultsData({ ...resultsData, cyl_blade_tilt_rh: e.target.value })}
                                    placeholder="Kondisi..."
                                    className="w-full bg-slate-950 border border-slate-700 rounded-lg px-2.5 py-1.5 text-white"
                                />
                            </div>
                            <div>
                                <label className="text-slate-400 text-[11px] block">Ripper Lift LEFT/RIGHT</label>
                                <input
                                    type="text"
                                    value={resultsData.cyl_ripper_lift_lh || ''}
                                    onChange={e => setResultsData({ ...resultsData, cyl_ripper_lift_lh: e.target.value })}
                                    placeholder="Kondisi..."
                                    className="w-full bg-slate-950 border border-slate-700 rounded-lg px-2.5 py-1.5 text-white"
                                />
                            </div>
                            <div>
                                <label className="text-slate-400 text-[11px] block">Ripper Tilt LEFT/RIGHT</label>
                                <input
                                    type="text"
                                    value={resultsData.cyl_ripper_tilt_lh || ''}
                                    onChange={e => setResultsData({ ...resultsData, cyl_ripper_tilt_lh: e.target.value })}
                                    placeholder="Kondisi..."
                                    className="w-full bg-slate-950 border border-slate-700 rounded-lg px-2.5 py-1.5 text-white"
                                />
                            </div>
                    ...
                    <div className="bg-slate-900/60 p-4 rounded-2xl border border-slate-800 flex flex-col justify-between">
                        ...
                        <div className="p-3 bg-slate-950 rounded-xl border border-slate-800 text-[11px] text-slate-400 mt-4">
                            ℹ️ Download data logging kejadian error code ECM dan parameter performa engine Dozer.
                        </div>
                    </div>
                </div>
                        ...
                        <div className="bg-slate-900 border border-slate-800 rounded-2xl max-w-2xl w-full p-5 shadow-2xl">
                            <div className="flex items-center justify-between pb-3 border-b border-slate-800">
                                <h3 className="text-sm font-bold text-white flex items-center gap-2">
                                    <span>🕒</span> Riwayat Check Sheet Service Dozer
                                </h3>
                                <button
                                    type="button"
                            ...
                            <div className="divide-y divide-slate-800 max-h-96 overflow-y-auto mt-3">
                                {recentForms.length === 0 ? (
                                    <div className="py-6 text-center text-xs text-slate-500">
                                        Belum ada data check sheet Dozer yang tersimpan.
                                    </div>
                                ) : (
                                    recentForms.map(rf => (
                                        <div key={rf.id} className="py-2.5 flex items-center justify-between">
                                            ...
                                            <div className="flex items-center gap-2">
                                                <Link
                                                    href={`/form-check-sheet-dozer?id=${rf.id}`}
                                                    className="px-2.5 py-1 rounded-lg bg-amber-600 hover:bg-amber-500 text-white text-[11px] font-semibold"
                                                >
                                                    Buka
                                                </Link>
                                                <a
                                                    href={`/form-check-sheet-dozer/${rf.id}/print`}
                                                    target="_blank"
                                                    rel="noopener noreferrer"
