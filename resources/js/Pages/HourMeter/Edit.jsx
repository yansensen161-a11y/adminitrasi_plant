...
export default function Edit({ log = {}, units = [] }) {
    const { data, setData, put, processing, errors } = useForm({
        code_unit: log?.code_unit || '',
        log_date: log?.log_date || '',
        hm_start: log?.hm_start || 0,
        hm_end: log?.hm_end || 0,
        hm_total: log?.hm_total || 0,
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        put(route('hour-meters.update', log.id));
    };

    return (
        <AuthenticatedLayout header={`Edit Log HM: ${log.code_unit} (${log.log_date})`}>
            <Head title={`Edit Log HM ${log.code_unit}`} />
