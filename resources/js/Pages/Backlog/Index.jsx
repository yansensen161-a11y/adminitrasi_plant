        ...
        <AuthenticatedLayout>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                <div className="bg-white p-4 rounded-xl border border-gray-100 shadow-sm flex items-center gap-4">
                    ...
                    <div>
                        <div className="text-[10px] font-bold text-gray-400 uppercase tracking-wide">TOTAL BACKLOG</div>
                ...
                <div className="bg-white p-4 rounded-xl border border-gray-100 shadow-sm flex items-center gap-4">
                    ...
                    <div>
                        <div className="text-[10px] font-bold text-gray-400 uppercase tracking-wide">BACKLOG RINGAN (1-2 SERVICE)</div>
                        <div className="text-2xl font-black text-gray-900">{stats?.ringan || 8}</div>
                        <div className="text-[10px] text-gray-500 font-semibold">Unit ({stats?.ringan_pct?.toString().replace('.', ',')}%)</div>
                    </div>
                </div>
                ...
                <div className="bg-white p-4 rounded-xl border border-gray-100 shadow-sm flex items-center gap-4">
                    ...
                    <div>
                        <div className="text-[10px] font-bold text-gray-400 uppercase tracking-wide">BACKLOG SEDANG (3-4 SERVICE)</div>
                        <div className="text-2xl font-black text-gray-900">{stats?.sedang || 9}</div>
                        <div className="text-[10px] text-gray-500 font-semibold">Unit ({stats?.sedang_pct?.toString().replace('.', ',')}%)</div>
                    </div>
                </div>
                ...
                <div className="bg-white p-4 rounded-xl border border-gray-100 shadow-sm flex items-center gap-4">
                    ...
                    <div>
                        <div className="text-[10px] font-bold text-gray-400 uppercase tracking-wide">BACKLOG BERAT (+4 SERVICE)</div>
                        <div className="text-2xl font-black text-gray-900">{stats?.berat || 6}</div>
                        <div className="text-[10px] text-gray-500 font-semibold">Unit ({stats?.berat_pct?.toString().replace('.', ',')}%)</div>
                    </div>
                </div>
            </div>
            ...
            <div className="bg-white shadow-sm sm:rounded-xl mb-4 p-4 border border-gray-100">
                <form onSubmit={handleFilterSubmit} className="flex flex-col md:flex-row gap-4 items-end">
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-4 flex-1 w-full">
                        ...
                        <div>
                            <label className="block text-[11px] font-bold text-gray-800 mb-1">Tingkat Backlog</label>
                            <select
                                value={tingkatFilter}
                    ...
                    <div className="flex flex-col gap-2 w-full md:w-auto">
                        <button
                            type="submit"
                            className="bg-[#0a4d3c] hover:bg-[#08422c] text-white font-bold px-6 py-2 rounded-lg text-xs transition flex items-center justify-center gap-2 h-9"
                        >
                        ...
                        <button
                            type="button"
                            onClick={handleReset}
                            className="bg-white hover:bg-gray-50 text-gray-700 font-bold px-6 py-2 rounded-lg text-xs transition border border-gray-200 flex items-center justify-center gap-2 h-9"
                        >
            ...
            <div className="bg-blue-50/50 border border-blue-100 rounded-xl p-3 mb-6 flex items-start gap-3">
                <div className="w-5 h-5 rounded-full bg-blue-500 text-white flex items-center justify-center font-bold shrink-0 mt-0.5 text-xs">
                    i
                </div>
            ...
            {/* Table */}
            <h2 className="text-xs font-extrabold text-[#0b5c3e] uppercase tracking-wider mb-3">DAFTAR BACKLOG</h2>
                            ...
                            {backlogs.data && backlogs.data.length > 0 ? (
                                backlogs.data.map((item, index) => (
                                    <tr key={item.id} className="hover:bg-gray-50 transition-colors">
                                        ...
                                        <td className="px-3 py-2 text-gray-600 whitespace-pre-line text-[10px]">
                                            {item.tindakan_mekanik}
                                        </td>
                                        <td className="px-3 py-2 text-center">
                                            {getTingkatBadge(item.tingkat_backlog)}
                                        </td>
                                        <td className="px-3 py-2 text-gray-700 whitespace-pre-line font-medium text-[10px]">
                                            {item.target_pasang}
                                        </td>
                                        <td className="px-3 py-2 text-center text-gray-600">
                                            {item.tanggal_temuan}
