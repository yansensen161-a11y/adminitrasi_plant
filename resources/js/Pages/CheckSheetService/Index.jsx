...
export default function Index({
    units = [],
    defaultItems = [],
    suggestedFormNumber = 'PLT/FRM/CSS/001',
    selectedForm = null,
    recentForms = [],
    prefill = {},
}) {
    ...
    // Handle Item Status Toggle (OK, REPAIR, etc)
    ...
    const handleResetAll = () => {
        if (window.confirm('Reset semua status checklist form ini?')) {
    ...
    const handleSave = (e) => {
        ...
        if (selectedForm && selectedForm.id) {
            router.put(`/form-check-sheet-service/${selectedForm.id}`, payload, {
                onSuccess: () => {
                    setIsSaving(false);
                    setNotification('Check Sheet Service berhasil diperbarui.');
                    setTimeout(() => setNotification(null), 4000);
                },
                onError: () => setIsSaving(false),
            });
        } else {
            router.post('/form-check-sheet-service', payload, {
                onSuccess: () => {
                    setIsSaving(false);
                    setNotification('Check Sheet Service berhasil disimpan.');
                    setTimeout(() => setNotification(null), 4000);
                },
                onError: () => setIsSaving(false),
            });
    ...
    return (
        <AuthenticatedLayout>
            <Head title="Check Sheet Service" />
            ...
            <div className="py-6 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto space-y-6">
                ...
                <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 bg-slate-900/60 p-5 rounded-2xl border border-slate-800 backdrop-blur-xl">
                    <div>
                        <div className="flex items-center gap-3">
                            <span className="p-2 rounded-xl bg-indigo-500/10 text-indigo-400 border border-indigo-500/20 text-xl font-bold">📋</span>
                            <div>
                                <h1 className="text-2xl font-black tracking-tight text-white flex items-center gap-2">
                                    CHECK SHEET SERVICE
                                </h1>
                                <p className="text-xs text-slate-400 font-medium mt-0.5">
                                    Form Inspeksi & Checklist Service Berkala Alat Berat (Hauler / Dump Truck / General)
                                </p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="flex flex-wrap items-center gap-2">
                        ...
                        {/* Blank Print */}
                        <a
                            href={`/form-check-sheet-service/blank-print${unitId ? `?unit_id=${unitId}` : ''}`}
                            target="_blank"
                        ...
                        {selectedForm && (
                            <>
                                <a
                                    href={`/form-check-sheet-service/${selectedForm.id}/print`}
                                    target="_blank"
                                    rel="noopener noreferrer"
                                ...
                                <a
                                    href={`/form-check-sheet-service/download-pdf?id=${selectedForm.id}`}
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4 bg-slate-900/60 p-5 rounded-2xl border border-slate-800">
                    ...
                    <div>
                        <label className="block text-xs font-semibold text-slate-300 mb-1.5">Pilih Unit Alat Berat *</label>
                        <select
                            value={unitId}
                        ...
                        <table className="w-full text-left text-xs text-slate-300">
                            <thead className="bg-slate-950 text-slate-400 border-b border-slate-800 uppercase font-bold text-[10px] tracking-wider">
                                <tr>
                                    ...
                                    <th className="px-4 py-2.5">Oil Kompartemen / Komponen</th>
                                ...
                                {itemsState.map((item, idx) => {
                                    ...
                                    return (
                                        ...
                                        >
                                            ...
                                            {/* Interval cells with Blackout Indicator */}
                                            {['250/750', '500', '1000', '2000'].map(intv => {
                                                ...
                                                return (
                                                    <td key={intv} className={`px-2 py-2 text-center ${isSelected ? 'bg-indigo-950/30' : ''}`}>
                                                        <button
                                                            type="button"
                                                            ...
                                                            className={`w-6 h-6 rounded-md border text-xs font-bold transition inline-flex items-center justify-center ${
                                                                item.status === 'OK' && isSelected
                                                                    ...
                                                                    : isSelected
                                                                    ? 'bg-emerald-950/40 border-emerald-600/50 text-emerald-400 hover:bg-emerald-600 hover:text-white'
                ...
                {/* Specialized Grid: Magnetic Plug, Cut Filter, Cylinder, Data Download */}
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {/* Magnetic Plug Card */}
                    <div className="bg-slate-900/60 p-4 rounded-2xl border border-slate-800">
                        <div className="flex items-center justify-between mb-3 border-b border-slate-800 pb-2">
                            <h3 className="text-xs font-bold text-white uppercase tracking-wider flex items-center gap-1.5">
                                <span>🧲</span> Plug Magnet & Catat (Magnetic Plug for Particles)
                            </h3>
                    ...
                    <div className="bg-slate-900/60 p-4 rounded-2xl border border-slate-800">
                        <div className="flex items-center justify-between mb-3 border-b border-slate-800 pb-2">
                            <h3 className="text-xs font-bold text-white uppercase tracking-wider flex items-center gap-1.5">
                                <span>✂️</span> Potong / Periksa Saringan Bermasalah
                            </h3>
                            <span className="text-[10px] text-blue-400 italic">STP Filter Cut</span>
                        </div>
                        <div className="grid grid-cols-2 gap-2.5 text-xs">
                            <div>
                                <label className="text-slate-400 text-[11px] block">Engine Filter Cut</label>
                                <input
                                    type="text"
                                    value={resultsData.cut_engine || ''}
                                    onChange={e => setResultsData({ ...resultsData, cut_engine: e.target.value })}
                                    placeholder="Normal / Serpihan..."
                                    className="w-full bg-slate-950 border border-slate-700 rounded-lg px-2.5 py-1.5 text-white"
                                />
                            </div>
                            <div>
                                <label className="text-slate-400 text-[11px] block">Hydraulic Filter Cut</label>
                                <input
                                    type="text"
                            ...
                            <div>
                                <label className="text-slate-400 text-[11px] block">Transmission Filter Cut</label>
                                <input
                                    type="text"
                            ...
                            <div>
                                <label className="text-slate-400 text-[11px] block">Steering Filter Cut</label>
                                <input
                                    type="text"
                                    value={resultsData.cut_steering || ''}
                                    onChange={e => setResultsData({ ...resultsData, cut_steering: e.target.value })}
                                    placeholder="Normal / Serpihan..."
                                    className="w-full bg-slate-950 border border-slate-700 rounded-lg px-2.5 py-1.5 text-white"
                                />
                            </div>
                    ...
                    <div className="bg-slate-900/60 p-4 rounded-2xl border border-slate-800">
                        <div className="flex items-center justify-between mb-3 border-b border-slate-800 pb-2">
                            <h3 className="text-xs font-bold text-white uppercase tracking-wider flex items-center gap-1.5">
                                <span>🔧</span> Periksa & Catat Cylinder-Cylinder
                            </h3>
                            <span className="text-[10px] text-blue-400 italic">STP Cylinder</span>
                        </div>
                        <div className="grid grid-cols-2 gap-2.5 text-xs">
                            <div>
                                <label className="text-slate-400 text-[11px] block">Hoist RH / Front Strut RH</label>
                                <input
                                    type="text"
                                    value={resultsData.cyl_hoist_rh || ''}
                                    onChange={e => setResultsData({ ...resultsData, cyl_hoist_rh: e.target.value })}
                                    placeholder="Good / Scratch..."
                                    className="w-full bg-slate-950 border border-slate-700 rounded-lg px-2.5 py-1.5 text-white"
                                />
                            </div>
                            <div>
                                <label className="text-slate-400 text-[11px] block">Hoist LH / Front Strut LH</label>
                                <input
                                    type="text"
                                    value={resultsData.cyl_hoist_lh || ''}
                                    onChange={e => setResultsData({ ...resultsData, cyl_hoist_lh: e.target.value })}
                                    placeholder="Good / Scratch..."
                                    className="w-full bg-slate-950 border border-slate-700 rounded-lg px-2.5 py-1.5 text-white"
                                />
                            </div>
                    ...
                    {/* Operational Data Acquisition & ET */}
                    <div className="bg-slate-900/60 p-4 rounded-2xl border border-slate-800">
                        <div className="flex items-center justify-between mb-3 border-b border-slate-800 pb-2">
                            <h3 className="text-xs font-bold text-white uppercase tracking-wider flex items-center gap-1.5">
                                <span>💻</span> Pengambilan Data Modul & Testing
                            </h3>
                            <span className="text-[10px] text-blue-400 italic">STP Electronic Download</span>
                        </div>
                        <div className="grid grid-cols-3 gap-2.5 text-xs">
                            <div>
                                <label className="text-slate-400 text-[11px] block">Download DATA ET</label>
                                <input
                                    type="text"
                                    value={resultsData.et_download || ''}
                                    onChange={e => setResultsData({ ...resultsData, et_download: e.target.value })}
                                    placeholder="Completed / N/A"
                                    className="w-full bg-slate-950 border border-slate-700 rounded-lg px-2.5 py-1.5 text-white"
                                />
                            </div>
                            <div>
                                <label className="text-slate-400 text-[11px] block">Engine High Idle</label>
                                <input
                                    type="text"
                                    value={resultsData.eng_high_idle || ''}
                                    onChange={e => setResultsData({ ...resultsData, eng_high_idle: e.target.value })}
                                    placeholder="RPM"
                                    className="w-full bg-slate-950 border border-slate-700 rounded-lg px-2.5 py-1.5 text-white"
                                />
                            </div>
                            <div>
                                <label className="text-slate-400 text-[11px] block">Engine Low Idle</label>
                                <input
                                    type="text"
                                    value={resultsData.eng_low_idle || ''}
                                    onChange={e => setResultsData({ ...resultsData, eng_low_idle: e.target.value })}
                                    placeholder="RPM"
                                    className="w-full bg-slate-950 border border-slate-700 rounded-lg px-2.5 py-1.5 text-white"
                                />
                            </div>
                            <div>
                                <label className="text-slate-400 text-[11px] block">Service Brake Test</label>
                                <input
                                    type="text"
                                    value={resultsData.brake_service || ''}
                                    onChange={e => setResultsData({ ...resultsData, brake_service: e.target.value })}
                                    placeholder="Pass / Fail"
                                    className="w-full bg-slate-950 border border-slate-700 rounded-lg px-2.5 py-1.5 text-white"
                                />
                            </div>
                            <div>
                                <label className="text-slate-400 text-[11px] block">Parking Brake Test</label>
                                <input
                                    type="text"
                                    value={resultsData.brake_parking || ''}
                                    onChange={e => setResultsData({ ...resultsData, brake_parking: e.target.value })}
                                    placeholder="Pass / Fail"
                                    className="w-full bg-slate-950 border border-slate-700 rounded-lg px-2.5 py-1.5 text-white"
                                />
                            </div>
                            <div>
                                <label className="text-slate-400 text-[11px] block">Hoist Cycle Time</label>
                                <input
                                    type="text"
                                    value={resultsData.hoist_time || ''}
                                    onChange={e => setResultsData({ ...resultsData, hoist_time: e.target.value })}
                                    placeholder="Detik"
                                    className="w-full bg-slate-950 border border-slate-700 rounded-lg px-2.5 py-1.5 text-white"
                                />
                            </div>
                        ...
                        <div className="bg-slate-900 border border-slate-800 rounded-2xl max-w-2xl w-full p-5 shadow-2xl">
                            <div className="flex items-center justify-between pb-3 border-b border-slate-800">
                                <h3 className="text-sm font-bold text-white flex items-center gap-2">
                                    <span>🕒</span> Riwayat Check Sheet Service Terakhir
                                </h3>
                                <button
                                    type="button"
                            ...
                            <div className="divide-y divide-slate-800 max-h-96 overflow-y-auto mt-3">
                                {recentForms.length === 0 ? (
                                    <div className="py-6 text-center text-xs text-slate-500">
                                        Belum ada data check sheet service yang tersimpan.
                                    </div>
                                ) : (
                                    recentForms.map(rf => (
