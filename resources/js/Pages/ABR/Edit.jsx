...
export default function Edit({ auth, units, abr }) {
    ...
    const extractAndPad = (category, defaults = {}) => {
        const existing = abr.items.filter(i => i.category === category).map(mapItem);
    ...
    const [manpowerItems, setManpowerItems] = useState(extractAndPad('manpower', { qty: 2, satuan: '2' }));
    const [sparepartItems, setSparepartItems] = useState(extractAndPad('sparepart', { satuan: 'Pcs' }));
    const [evakuasiItems, setEvakuasiItems] = useState(extractAndPad('evakuasi', { qty: 1, satuan: '1' }));
    ...
    const existingDisassembly = abr.items.filter(i => i.category === 'disassembly').map(mapItem);
    ...
    return (
        <AuthenticatedLayout
            ...
            header={<div className="flex justify-between items-center">
                <h2 className="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">Edit ABR - {abr.no_abr}</h2>
                <div className="text-sm text-gray-500">ABR / Edit</div>
            </div>}
        >
            ...
            <div className="py-8 bg-gray-200">
                <form onSubmit={handleSubmit} className="max-w-[210mm] mx-auto bg-white p-8 shadow-xl relative" style={{ fontFamily: 'Arial, sans-serif' }}>
                    ...
                    <div className="absolute -top-12 right-0 flex gap-2">
                        ...
                        <button type="submit" className="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded shadow">Update Dokumen</button>
                    </div>
                            ...
                            <div className="mb-6">
                                ...
                                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                                    {existingImages.map((img) => (
                                        <div key={img.id} className="relative border rounded p-1">
                                            <img src={img.file_path} alt="Existing" className="w-full h-32 object-cover rounded" />
                                            <button type="button" onClick={() => deleteExistingImage(img.id)} className="absolute top-2 right-2 bg-red-500 text-white rounded-full w-6 h-6 flex items-center justify-center hover:bg-red-600 shadow">&times;</button>
                                        </div>
                                    ))}
                                </div>
                            </div>
