...
export default function Edit({ auth, units, abr }) {
    ...
    const mapItem = (item) => {
        const mapped = { ...item };
        if (mapped.category === 'manpower' || mapped.category === 'disassembly' || mapped.category === 'evakuasi') {
            if (!mapped.qty && mapped.hour) mapped.qty = mapped.hour;
            if (!mapped.satuan && mapped.mp) mapped.satuan = mapped.mp;
        }
        return mapped;
    };
    ...
    const extractAndPad = (category, defaults = {}) => {
        const existing = abr.items.filter(i => i.category === category).map(mapItem).map(item => ({
            ...item,
            _key: item.id || generateRowKey()
        }));
        if (existing.length === 0) {
            return [createEmptyRow(category, defaults)];
        }
        return existing;
    };

    const [repairItems, setRepairItems] = useState(extractAndPad('repair', { satuan: 'Set' }));
    const [manpowerItems, setManpowerItems] = useState(extractAndPad('manpower'));
    const [sparepartItems, setSparepartItems] = useState(extractAndPad('sparepart', { satuan: 'Pcs' }));
    const [evakuasiItems, setEvakuasiItems] = useState(extractAndPad('evakuasi'));
    
    const defaultDisassembly = [
        createEmptyRow('disassembly')
    ];
    const existingDisassembly = abr.items.filter(i => i.category === 'disassembly').map(mapItem).map(item => ({
        ...item,
        _key: item.id || generateRowKey()
    }));
    const [disassemblyItems, setDisassemblyItems] = useState(existingDisassembly.length > 0 ? existingDisassembly : defaultDisassembly);
    ...
    const deleteExistingImage = (imageId) => {
        if (confirm('Apakah Anda yakin ingin menghapus foto ini?')) {
            router.delete(route('abr.images.destroy', imageId), {
                preserveScroll: true,
                onSuccess: () => {
    ...
    const handleSubmit = (e) => {
        ...
        router.post(route('abr.update', abr.id), formData, {
            forceFormData: true,
            preserveScroll: true,
    ...
    return (
        <AuthenticatedLayout user={auth.user}>
            <Head title={`Edit ABR ${abr.no_abr}`} />
            ...
            <div className="flex flex-col bg-gray-100 dark:bg-transparent min-h-screen">
                ...
                <div className="bg-white dark:bg-[#060b14] px-6 py-4 flex items-center justify-between shadow-sm border-b border-gray-200 dark:border-white/10">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h1 className="text-xl font-black text-[#012922] dark:text-white tracking-tight">Edit ABR</h1>
                            <div className="flex items-center text-sm text-gray-500 font-medium gap-1">
                                ...
                                <span>›</span>
                                <span className="text-orange-600 font-bold">Edit — {abr.no_abr}</span>
                            </div>
                        </div>
                    </div>
                    <div className="flex items-center gap-2">
                        <Link href={route('abr.index')} className="bg-gray-100 hover:bg-gray-200 border border-gray-300 text-gray-700 font-bold py-2 px-4 rounded shadow-sm text-sm transition">Batal</Link>
                        <button
                            type="submit"
                            form="abr-edit-form"
                        ...
                        >
                            ...
                            <span>{isSubmitting ? 'Memperbarui...' : 'Update Dokumen'}</span>
                        </button>
                    </div>
                </div>

                <div className="p-6">
                <form id="abr-edit-form" onSubmit={handleSubmit} className="abr-paper w-full bg-white p-8 shadow-xl relative" style={{ fontFamily: 'Arial, sans-serif' }}>
                    <style>
                        {`
                            .abr-paper {
                    ...
                    <div className="mt-8 border-t-2 border-dashed border-gray-300 pt-6">
                        ...
                        {existingImages.length > 0 && (
                            <div className="mb-6">
                                <h4 className="text-sm font-semibold mb-2 text-gray-700">Foto Saat Ini:</h4>
                                <div className={existingImages.length === 1 ? "max-w-md mx-auto" : "grid grid-cols-2 md:grid-cols-4 gap-4"}>
                                    {existingImages.map((img) => (
                                        <div key={img.id} className="relative border rounded p-1.5 bg-gray-50 flex items-center justify-center">
                                            <img src={img.file_path} alt="Existing" className={`rounded ${existingImages.length === 1 ? 'w-auto max-h-80 object-contain mx-auto' : 'w-full h-32 object-cover'}`} />
                                            <button type="button" onClick={() => deleteExistingImage(img.id)} className="absolute top-2 right-2 bg-red-500 text-white rounded-full w-6 h-6 flex items-center justify-center hover:bg-red-600 shadow" title="Hapus foto">&times;</button>
                                        </div>
                                    ))}
                                </div>
                            </div>
                        ...
                        <div className="mb-4">
                            <h4 className="text-sm font-semibold mb-2 text-gray-700">Tambah Foto Baru:</h4>
                            ...
                            <div className={imagePreviews.length === 1 ? "max-w-md mx-auto" : "grid grid-cols-2 md:grid-cols-4 gap-4"}>
                                {imagePreviews.map((preview, idx) => (
                                    <div key={idx} className="relative border rounded p-1.5 bg-gray-50 flex items-center justify-center">
                                        ...
                                        <button type="button" onClick={() => removeNewImage(idx)} className="absolute top-2 right-2 bg-red-500 text-white rounded-full w-6 h-6 flex items-center justify-center hover:bg-red-600 shadow">&times;</button>
                                    </div>
                                ))}
                            </div>
                        )}
